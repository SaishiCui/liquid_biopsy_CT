...
Program: Step_D: Predicting_Models_withoutTCR.py
Purpose: Predicting models using deconvolution results without TCR.
--------------------------------------------------------------------------------
Data Inputs:
- Deconvolution results (without TCR) obtained from Step_D: Deconvolution_xxx_et_al_withoutTCR.py under the folder "Pipeline".

Data Outputs:
- ROC curves of predicting models using deconvolution results without TCR.
- Logistic regression to predict cancer vs normal using deconvolution results without TCR.
- Machine learning models using deconvolution results without TCR.
--------------------------------------------------------------------------------
"""

nnls_df_Lee_withoutTCR = pd.read_csv('/Users/scui2/DNAmethylation/Lee_et_al/nnls_df_Lee_withoutTCR.csv')
nnls_df_Li_withoutTCR = pd.read_csv('/Users/scui2/DNAmethylation/Li_et_al/nnls_df_Li_withoutTCR.csv')
nnls_df_Wang_withoutTCR = pd.read_csv('/Users/scui2/DNAmethylation/Wang_et_al/nnls_df_Wang_withoutTCR.csv')
nnls_df_Xie_withoutTCR = pd.read_csv('/Users/scui2/DNAmethylation/Xie_et_al/nnls_df_Xie_withoutTCR.csv')

nnls_df_combined_withoutTCR = pd.concat([nnls_df_Lee_withoutTCR, nnls_df_Li_withoutTCR, nnls_df_Wang_withoutTCR, nnls_df_Xie_withoutTCR], axis = 0)

nnls_df_combined_withoutTCR["Group"][nnls_df_combined_withoutTCR["Group"] != "Normal"] = "Cancer"
nnls_df_combined_withoutTCR["Group"][nnls_df_combined_withoutTCR["Group"] == "Normal"] = "Normal"


####### Logistic regression
### CD4+T 

X_Xie = nnls_df_Xie_withoutTCR.iloc[:, [0,1,2,3,4]]
y_Xie = np.array(['Cancer'] * 50 + ['Normal'] * 50)
y_num_Xie = (y_Xie == 'Cancer').astype(int)


X_Wang = nnls_df_Wang_withoutTCR.iloc[:, [0,1,2,3,4]]
y_Wang = np.array(['Cancer'] * 50 + ['Normal'] * 30)
y_num_Wang = (y_Wang == 'Cancer').astype(int)


X_Li = nnls_df_Li_withoutTCR.iloc[:, [0,1,2,3,4]]
y_Li = np.array(['Cancer'] * 35 + ['Normal'] * 50)
y_num_Li = (y_Li == 'Cancer').astype(int)

X_Lee = nnls_df_Lee_withoutTCR.iloc[:, [0,1,2,3,4]]
...
X_Combined_Xie = X_Xie[["CD4+T", "CD8+T", "B", "Mono", "NK"]].values
...
X_Combined_Wang = X_Wang[["CD4+T", "CD8+T", "B", "Mono", "NK"]].values
...
X_Combined_Li = X_Li[["CD4+T", "CD8+T", "B", "Mono", "NK"]].values
...
X_Combined_Lee = X_Lee[["CD4+T", "CD8+T", "B", "Mono", "NK"]].values
...
########### Combined 4 studies and joint model 
...
X_Xie_dropped = X_Xie.drop(columns=["B"])
X_Wang_dropped = X_Wang.drop(columns=["B"])
X_Li_dropped = X_Li.drop(columns=["B"])
X_Lee_dropped = X_Lee.drop(columns=["B"])

X_combined = np.concatenate([X_Xie_dropped, X_Wang_dropped, X_Li_dropped], axis=0)
...
feature_names_Combined = ['Intercept'] + ["CD4+T", "CD8+T", "Mono", "NK", "Study_Xie", 'Study_Wang', 'Study_Li']
...
X_train, X_test, y_train, y_test = train_test_split(X_combined, y_num_combined, test_size=0.3, random_state=42)
